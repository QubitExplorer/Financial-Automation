...
data_directory = r'D:\Stocks\Execute_10_1000'  # Replace with your directory path
input_excel = 'D:\Stocks\Pull_List.xlsx'  # Replace with your input Excel file name
df = pd.read_excel(input_excel, usecols=[0,6])
stocks_list = df['Symbol'].tolist()  # Use the actual header name for the first column
#stocks_list = stocks_list[:30]
...
for count, stock in enumerate(stocks_list, start=1):
    try:
            ...
            for i, row in df.iterrows():
                ...
                if row['counter'] > 365 and row['% Difference'] < sell_target and prev_buy_period > 0 and holdings == 'Buy':
                ...
                elif row['counter'] > 365 and row['% Difference'] < buy_target and prev_buy_period == 0 and holdings == '' and not pd.isna(ltd1_int) and ltd1_int > 365:
                ...
                elif row['counter'] > 365 and row['% Difference'] < stop_loss and prev_buy_period > 0 and holdings == 'Buy':
                ...
                elif row['counter'] > 365 and row['% Difference'] < stop_loss and prev_buy_period > 0:
                        # This effectively checks if the previous signal was not 'buy'
                        df.at[i, 'signal'] = ''
                        df.at[i, 'Buy Date'] = ''
                        holdings = ''
